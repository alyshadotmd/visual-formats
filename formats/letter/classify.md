...
description: How to identify whether an ad should be classified/named as the Letter visual format. Use when naming an ad (resolving the Visual Format taxonomy token) or auditing an existing ad's format tag. Distinct from execute.md, which governs how to build one.
---
...
1. **It's an image, never a video.** (A person reading or confessing on camera is `founder`, not Letter.)
...
- **A salutation and sign-off** that make it read like a real letter. Characteristic, but the hard requirement is the headline + text block, not a formal greeting.

## Quick test

Is it an image whose main event is a chunky block of text with one headline pulling you in to read it? If yes, Letter. If it's a talking-head video, a short sticky-note line, or a headline-and-CTA banner, it's a different format.

## Commonly confused with

- **Founder:** same first-person origin/belief/apology content, but Founder is **spoken on camera (video)**. Letter is a written text block in an **image**. If it's watched, not read, it's `founder`.
...
- A signature/role, sometimes a P.S. that carries the offer or the sharpest line
