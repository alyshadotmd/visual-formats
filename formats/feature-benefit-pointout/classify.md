---
name: feature-benefit-pointout-classification
description: How to identify whether an ad should be classified/named as the Feature Benefit Pointout visual format. Use when naming an ad (resolving the Visual Format taxonomy token) or auditing an existing ad's format tag. Distinct from execute.md, which governs how to build one.
---
...
If there's no product being annotated with multiple call-outs, it's not this format, even if it has some text overlay.

## Quick test

Ask: *does this look like an annotated diagram of one product, where each label points at a specific part and explains what that part does for you?* If yes, Feature Benefit Pointout.
